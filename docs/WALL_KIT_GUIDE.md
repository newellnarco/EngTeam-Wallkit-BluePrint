...
`agents.py` keeps the roster in `.wall/registry/agents.md` under an `O_EXCL` lock. **Architect, Adjudicator and Warden are singletons, refused in code at claim time.** The Foreman singleton is a lock file with a heartbeat and a TTL. Names that are confusable with a live agent's name are refused.
