...
|---|---|---|
| **Segregation of duties** — no actor authors, approves and ships alone | The authority matrix: the Builder cannot merge, the Maestro cannot edit source, the Reviewer alone reads code for correctness, the Foreman cannot assign work; enforced as written boundaries + role sheets, visible in every run record | WORKFLOW.md §7, AGENT_ROSTER_SPEC.md |
